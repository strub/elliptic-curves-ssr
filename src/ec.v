...
From mathcomp
Require Import zmodp fingroup.
Require Import polyall polydec ssrring SsrMultinomials.mpoly.
...
(* -------------------------------------------------------------------- *)
(* FIXME: move me                                                       *)
Lemma ltn_pred (n m : nat): (n.-1 < m.-1) -> (n < m).
Proof. by case: n m => [|n] [|m]. Qed.

Lemma maxn_pred n m: maxn n.-1 m.-1 = (maxn n m).-1.
Proof. by case: n m => [|n] [|m]; rewrite ?(maxn0, max0n, maxnSS). Qed.

Definition monotone f := forall m n, (f m <= f n) = (m <= n).

Lemma map_maxn f n m: monotone f -> f (maxn n m) = maxn (f n) (f m).
Proof.
  move=> mono_f; rewrite /maxn [f n < _]ltnNge mono_f.
  by rewrite -ltnNge; case: (_ < _).
Qed.
